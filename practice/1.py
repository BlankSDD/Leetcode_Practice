...
#     mid_point = defaultdict(list)
#     for i in range(n-1):
#         for j in range(i+1, n):
#             d = dis(points[i], points[j])
#             mp = mid(points[i], points[j])
#             mid_point[(d,mp)].append([points[i], points[j]])

#     ans = inf
#     for _, pps in mid_point.items():
#         n1 = len(pps)
#         if n1 < 2:
#             continue
#         for i in range(n1-1):
#             for j in range(i+1, n1):
#                 ans = min(ans, dis(pps[i][0],pps[j][0])*dis(pps[i][0],pps[j][1]))
...
#     ### solution 3: dp+循环生成数量变多的子树+创建新的树
#     if not n%2:
#         return []
...
#     ### solution 2: 递归左右子树的创建+结合根节点创建新的树
#     # if not n%2:
#     #     return []
...
#     #             leave.left = TreeNode()
#     #             leave.right = TreeNode()
#     #             dfs(t, [leave.left, leave.right]+leaves[i+1:], n-2)
#     #             leave.left = None
#     #             leave.right = None
#     #     else:
#     #         ans.append(copy.deepcopy(t))
    
#     # dfs(root, [root.left, root.right], n)
#     # return ans
...
#     tmp = []
#     for c in s:
#         if tmp and abs(ord(tmp[-1]) - ord(c)) == 32:
#             tmp.pop()
#         else:
#             tmp.append(c)
...
#     # for i in range(n-1, 0, -1):
#     #     if (num[i] == '5'):
#     #         j = max(num.rfind('7', 0, i), num.rfind('2', 0, i))
#     #         if j >= 0:
#     #             ans = min(ans, n-j-2)
#     #     elif (num[i] == '0'):
#     #         j = max(num.rfind('5', 0, i), num.rfind('0', 0, i))
#     #         if j >= 0:
#     #             ans = min(ans, n-j-2)
...
#     ### solution 2: 优化正向搜索"75", "50", "25", "00"
#     n = len(num)
#     ans = n
#     for i in range(n-1):
#         if (num[i] == '7' or num[i] == '2') and '5' in num[i+1:]:
#             ans = n-i-2
#         elif (num[i] == '5' or num[i] == '0') and '0' in num[i+1:]:
#             ans = n-i-2
...
#     ### solution 1: 暴力正向搜索"75", "50", "25", "00"
#     # n = len(num)
#     # ans = n
#     # for i in range(n-1):
#     #     for j in range(i+1,n):
#     #         if num[i]+num[j] in ["75", "50", "25", "00"]:
#     #             ans = min(ans, n-i-2)
#     #             break
...
#     dp = [0] * n
#     for i in range(1,n):
#         dp[i] = 1 + dp[i-1] + dp[i-1] - dp[nextVisit[i-1]] + 1
#         dp[i] %= mod
